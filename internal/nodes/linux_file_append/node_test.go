...
func TestTypeDef(t *testing.T) {
	def := (Node{}).TypeDef()
	if def.TypeID != "linux_file_append" {
		t.Fatalf("TypeID 不匹配: %s", def.TypeID)
	}
	if def.InputPorts[1].PortType != core.PortTypeLinuxFileHandle {
		t.Fatalf("handle 端口类型不匹配: %s", def.InputPorts[1].PortType)
	}
}

func TestExecuteRequiresHandle(t *testing.T) {
	_, err := (Node{}).Execute(fakeContext{config: map[string]any{"text": "hi"}})
	if err == nil || !strings.Contains(err.Error(), "缺少 handle 输入") {
		t.Fatalf("期望缺少 handle 错误，实际: %v", err)
	}
}

func TestExecuteRejectsWrongHandle(t *testing.T) {
	_, err := (Node{}).Execute(fakeContext{
		inputs:  map[string]any{"handle": "not handle"},
		handled: map[string]bool{"handle": true},
	})
	if err == nil || !strings.Contains(err.Error(), "handle 输入类型不是") {
		t.Fatalf("期望 handle 类型错误，实际: %v", err)
	}
}
