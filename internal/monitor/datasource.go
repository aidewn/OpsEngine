...
//   - 采集按 目标+类型+参数 去重后批量执行，避免监控项数量增长打爆服务器（§4.1）；
...
// Register 注册一个 DataSource；Kind 为空或重复均视为错误。
func (r *Registry) Register(ds DataSource) error {
	...
	if _, exists := r.sources[kind]; exists {
		return fmt.Errorf("DataSource 重复注册: %s", kind)
	}
	r.sources[kind] = ds
	return nil
}

// Get 按 Kind 取 DataSource。
func (r *Registry) Get(kind string) (DataSource, bool) {
	ds, ok := r.sources[kind]
	return ds, ok
}
